...
uint32_t allocate_frame(uint32_t process_id, uint32_t virtual_page_num) {
    ...
    if (victim_frame != (uint32_t)-1) {
        ...
        if (victim_process) {
            ...
            // 更新受害进程的页表
            uint32_t victim_vpn = memory_manager.frames[victim_frame].virtual_page_num;
            if (victim_vpn < victim_process->page_table_size) {
                victim_process->page_table[victim_vpn].flags.present = false;
                victim_process->page_table[victim_vpn].flags.swapped = true;
            }
        }
        ...
        printf("页框 %u 已分配给进程 %u\n", victim_frame, process_id);
        return victim_frame;
    }
...
uint32_t select_victim_frame(void) {
    ...
    for (uint32_t i = 0; i < PHYSICAL_PAGES; i++) {
        if (memory_manager.frames[i].is_allocated) {
            ...
            printf("检查页框 %u: PID=%u, 访问时间=%llu, 时间差=%llu, 脏标志=%d\n",
                   i,
                   memory_manager.frames[i].process_id,
                   memory_manager.frames[i].last_access_time,
                   time_diff,
                   memory_manager.frames[i].is_dirty);
            
            // 跳过系统进程（PID=0）的页框
            if (memory_manager.frames[i].process_id == 0) {
                continue;
            }
            
            if (!memory_manager.frames[i].is_dirty && 
                memory_manager.frames[i].last_access_time < oldest_access) {
                oldest_access = memory_manager.frames[i].last_access_time;
                victim_frame = i;
                printf("  找到更旧的未修改页框 %u (时间差=%llu)\n", i, time_diff);
            }
        }
    }
    ...
    if (victim_frame != (uint32_t)-1) {
        ...
        printf("  当前时间=%llu\n", current_time);
        printf("  时间差=%llu\n", current_time - memory_manager.frames[victim_frame].last_access_time);
    } else {
        printf("\n警告：未找到合适的牺牲页框\n");
    }
...
bool read_physical_memory(uint32_t frame, uint32_t offset, void* buffer, size_t size) {
    ...
    if (frame >= PHYSICAL_PAGES || offset + size > PAGE_SIZE || !buffer) {
        printf("读取物理内存失败：参数无效\n");
        return false;  // 无效的参数，返回失败
    }

    // 检查页框是否已被使用
    if (!phys_mem.frame_map[frame]) {
        printf("读取物理内存失败：页框未被使用\n");
        return false;  // 页框未被使用，返回失败
    }

    // 更新访问时间
    uint64_t old_time = memory_manager.frames[frame].last_access_time;
    memory_manager.frames[frame].last_access_time = get_current_time();
    printf("更新页框 %u 的访问时间：%llu -> %llu\n", 
           frame, 
           old_time,
           memory_manager.frames[frame].last_access_time);
...
bool write_physical_memory(uint32_t frame, uint32_t offset, const void* data, size_t size) {
    ...
    if (frame >= PHYSICAL_PAGES || offset + size > PAGE_SIZE || !data) {
        printf("写入物理内存失败：参数无效\n");
        return false;  // 无效的参数，返回失败
    }

    // 检查页框是否已被使用
    if (!phys_mem.frame_map[frame]) {
        printf("写入物理内存失败：页框未被使用\n");
        return false;  // 页框未被使用，返回失败
    }
    ...
    printf("更新页框 %u 的访问时间：%llu -> %llu（已标记为脏）\n", 
           frame, 
           old_time,
           memory_manager.frames[frame].last_access_time);
