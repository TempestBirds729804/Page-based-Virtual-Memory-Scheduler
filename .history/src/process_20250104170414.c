...
PCB* create_process(uint32_t priority, uint32_t code_pages, uint32_t data_pages) {
    ...
    if (free_frames < required_frames) {
        printf("空闲页框不足（需要%u个，当前只有%u个），尝试页面置换...\n", 
               required_frames, free_frames);
        
        // 计算需要置换的页面数
        uint32_t frames_to_swap = required_frames - free_frames;
        
        // 尝试从其他进程置换出页面
        for (uint32_t i = 0; i < frames_to_swap; i++) {
            uint32_t victim_frame = select_victim_page(NULL);  // 从所有进程中选择牺牲页
            if (victim_frame == (uint32_t)-1) {
                printf("页面置换失败，无法获取足够的物理页框\n");
                return NULL;
            }
            printf("成功置换出页框 %u\n", victim_frame);
        }
        ...
        // 重新检查空闲页框数
        free_frames = get_free_frames_count();
        printf("页面置换后的空闲页框数：%u\n", free_frames);
    }
    ...
    for (uint32_t i = 0; i < required_frames; i++) {
        ...
        if (frame == (uint32_t)-1) {
            printf("分配物理页框失败，尝试页面置换\n");
            // 尝试页面置换
            frame = select_victim_page(NULL);
            if (frame == (uint32_t)-1) {
                printf("页面置换失败，进程创建失败\n");
    ...
    // 检查是否需要抢占当前运行的进程
    check_preemption(final_process);
