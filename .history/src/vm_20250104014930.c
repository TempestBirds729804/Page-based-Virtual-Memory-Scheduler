...
void access_memory(PCB* process, uint32_t virtual_address, bool is_write) {
    ...
    if (page_num >= process->page_table_size) {
        printf("错误：访问地址 0x%x 超出进程页表范围\n", virtual_address);
        return;
    }
    
    PageTableEntry* pte = &process->page_table[page_num];
    
    printf("进程 %d 访问地址 0x%x (页号 %u, 偏移 0x%x)\n", 
           process->pid, virtual_address, page_num, offset);
    ...
    if (is_write) {
        ...
        printf("进程 %d 写入地址 0x%x\n", process->pid, virtual_address);
    } else {
        printf("进程 %d 读取地址 0x%x\n", process->pid, virtual_address);
    }
    
    // 更新对应页框的访问信息
    if (pte->flags.present) {
        update_frame_access_time(pte->frame_number);
        if (is_write) {
            memory_manager.frames[pte->frame_number].is_dirty = true;
        }
    }
}
...
bool handle_page_fault(PCB* process, uint32_t virtual_page) {
    ...
    vm_manager.stats.page_faults++;  // 增加缺页计数
    ...
    if (frame == (uint32_t)-1) {
        ...
        vm_manager.stats.page_replacements++;  // 增加页面替换计数
        ...
        if (memory_manager.frames[victim_frame].is_dirty) {
            vm_manager.stats.disk_writes++;  // 增加磁盘写入计数
            if (!swap_out_page(victim_frame)) {
                printf("页面换出失败\n");
                return false;
            }
            vm_manager.stats.pages_swapped_out++;  // 增加页面换出计数
        }
    ...
    pte->flags.present = true;     // 页面现在在内存中
    pte->flags.swapped = false;    // 页面不在交换区
    pte->last_access_time = get_current_time();
...
bool swap_out_page(uint32_t frame) {
    ...
    printf("页面成功写入交换区：进程=%u, 虚拟页号=%u, 交换块=%u\n", 
