    ...
    if (strcmp(token, "help") == 0) {
        ...
        if (token) {
            if (strcmp(token, "create") == 0) {
                ...
                token = strtok(NULL, " \n");  // 优先级
                if (token) {
                    cmd.args.priority = (uint32_t)strtoul(token, NULL, 0);
                    token = strtok(NULL, " \n");  // 代码段页数
                    if (token) {
                        cmd.args.code_pages = (uint32_t)strtoul(token, NULL, 0);
                    }
                    token = strtok(NULL, " \n");  // 数据段页数
                    if (token) {
                        cmd.args.data_pages = (uint32_t)strtoul(token, NULL, 0);
                    }
                }
    ...
    switch (cmd->type) {
        ...
        case CMD_APP_CREATE:
            process = create_process(
                cmd->args.priority ? cmd->args.priority : DEFAULT_PRIORITY,
