...
uint32_t select_victim_page(PCB* process) {
    ...
    for (uint32_t pid = 1; pid < scheduler.next_pid; pid++) {
        ...
        printf("进程 %u 当前物理页面数量: %u, 比例: %.2f%%\n", 
               pid, present_pages, physical_ratio);
        
        // 如果进程物理页面比例超过40%，则可以考虑置换
        if (physical_ratio > 40.0f) {
            printf("进程 %u 的物理页面比例较高，可以置换\n", pid);
            ...
            for (uint32_t i = 0; i < page_count; i++) {
                printf("尝试置换进程 %u 的第 %u 个最旧页面（虚拟页号=%u，物理页框=%u）\n",
    ...
    if (max_pages_process && max_present_pages > 0) {
        printf("从占用内存最多的进程 %u (页面数=%u) 中强制置换页面\n", 
        ...
        // 尝试每个页面
        for (uint32_t i = 0; i < page_count; i++) {
            printf("尝试强制置换进程 %u 的第 %u 个最旧页面（虚拟页号=%u，物理页框=%u）\n",
