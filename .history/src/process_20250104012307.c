...
void simulate_process_memory_access(uint32_t pid, uint32_t access_count) {
    ...
    for (uint32_t i = 0; i < access_count; i++) {
        ...
        if (rand() % 2) {
            // 读操作
            uint32_t value;
            if (access_memory(proc, addr, false) == 0) {
                printf("进程 %u 读取地址 0x%x 成功\n", pid, addr);
            }
        } else {
            // 写操作
            uint32_t value = rand();
            if (access_memory(proc, addr, true) == 0) {
...
void allocate_process_memory(uint32_t pid, uint32_t size, uint32_t flags) {
    ...
    for (uint32_t i = 0; i < pages_needed; i++) {
        ...
        proc->page_table[proc->page_table_size].flags.dirty = 0;
        proc->page_table[proc->page_table_size].flags.referenced = 0;
