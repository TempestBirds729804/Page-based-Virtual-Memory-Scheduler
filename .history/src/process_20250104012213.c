...
void simulate_process_memory_access(uint32_t pid, uint32_t access_count) {
    ...
    for (uint32_t i = 0; i < access_count; i++) {
        ...
        if (rand() % 2) {
            // 读操作
            uint32_t value;
            if (mem_read(pid, addr, &value) == 0) {
                printf("进程 %u 读取地址 0x%x 成功，值：%u\n", pid, addr, value);
            }
        } else {
            // 写操作
            uint32_t value = rand();
            if (mem_write(pid, addr, value) == 0) {
                printf("进程 %u 写入地址 0x%x 成功，值：%u\n", pid, addr, value);
            }
        }
    }
...
void allocate_process_memory(uint32_t pid, uint32_t size, uint32_t flags) {
    ...
    for (uint32_t i = 0; i < pages_needed; i++) {
        ...
        proc->page_table[proc->page_table_size].valid = 1;
        proc->page_table[proc->page_table_size].dirty = 0;
        proc->page_table[proc->page_table_size].referenced = 0;
