...
bool handle_page_fault(PCB* process, uint32_t virtual_page) {
    ...
    if (frame == (uint32_t)-1) {
        ...
        printf("  之前 - present=%d, swapped=%d, frame=%u\n",
               victim_pte->flags.present, victim_pte->flags.swapped, victim_pte->frame_number);
        ...
        // 先将页框标记为未分配，这样它就不会被select_victim_frame选中
        memory_manager.frames[frame].is_allocated = false;
        memory_manager.free_frames_count++;
