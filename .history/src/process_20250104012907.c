...
PCB* create_process_with_pid(uint32_t pid, uint32_t priority, uint32_t code_pages, uint32_t data_pages) {
    ...
    if (code_pages == 0 || data_pages == 0 || 
        code_pages + data_pages > MAX_PAGES_PER_PROCESS) {
        printf("错误：页面数量不合理\n");
        return NULL;
    }
    ...
    new_process->page_table = (PageTableEntry*)malloc(sizeof(PageTableEntry) * MAX_PAGES_PER_PROCESS);
    if (!new_process->page_table) {
        free(new_process);
        printf("错误：页表分配失败\n");
        return NULL;
    ...
    // 初始化页表
    memset(new_process->page_table, 0, sizeof(PageTableEntry) * MAX_PAGES_PER_PROCESS);
    new_process->page_table_size = total_pages;
    ...
    new_process->memory_layout.heap.start_page = total_pages;
    new_process->memory_layout.heap.num_pages = 0;
    new_process->memory_layout.heap.is_allocated = false;
    
    new_process->memory_layout.stack.start_page = MAX_PAGES_PER_PROCESS - 1;
    new_process->memory_layout.stack.num_pages = 0;
    new_process->memory_layout.stack.is_allocated = false;
    ...
    for (uint32_t i = 0; i < total_pages; i++) {
        ...
        new_process->page_table[i].frame_number = frame;
        new_process->page_table[i].flags.present = 1;
        new_process->page_table[i].flags.swapped = 0;
