...
PCB* create_application(const char* name, AppConfig* config) {
    ...
    // 转换内存布局
    process->memory_layout.code.start_page = config->layout.code_start / PAGE_SIZE;
    process->memory_layout.code.num_pages = config->layout.code_size / PAGE_SIZE;
    process->memory_layout.code.is_allocated = true;
    
    process->memory_layout.data.start_page = config->layout.data_start / PAGE_SIZE;
    process->memory_layout.data.num_pages = config->layout.data_size / PAGE_SIZE;
    process->memory_layout.data.is_allocated = true;
    
    process->memory_layout.heap.start_page = config->layout.heap_start / PAGE_SIZE;
    process->memory_layout.heap.num_pages = config->layout.heap_size / PAGE_SIZE;
    process->memory_layout.heap.is_allocated = false;
    
    process->memory_layout.stack.start_page = config->layout.stack_start / PAGE_SIZE;
    process->memory_layout.stack.num_pages = config->layout.stack_size / PAGE_SIZE;
...
PCB* create_process(uint32_t priority, uint32_t code_pages, uint32_t data_pages) {
    ...
    // 复制进程信息到数组
    processes[process_index].pid = new_process->pid;
    processes[process_index].priority = new_process->priority;
    processes[process_index].state = new_process->state;
    processes[process_index].time_slice = new_process->time_slice;
    processes[process_index].page_table = new_process->page_table;
    processes[process_index].page_table_size = new_process->page_table_size;
