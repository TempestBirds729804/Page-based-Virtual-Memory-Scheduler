...
void handle_command(Command* cmd) {
    ...
    switch (cmd->type) {
        ...
        case CMD_APP_CREATE:
            // 解析参数：优先级 代码段页数 数据段页数
            uint32_t priority = DEFAULT_PRIORITY;
            uint32_t code_pages = 1;  // 默认值
            uint32_t data_pages = 1;  // 默认值
            
            if (cmd->args.text) {
                sscanf(cmd->args.text, "%u %u %u", &priority, &code_pages, &data_pages);
            }
            
            process = create_process(priority, code_pages, data_pages);
            if (!process) {
                printf("创建进程失败\n");
                return;
            }
