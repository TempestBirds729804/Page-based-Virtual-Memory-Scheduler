...
void add_to_ready_queue(PCB* process) {
    ...
    if (priority >= 3) priority = 2;  // 将超出0-2范围的优先级设为2
...
PCB* create_process(uint32_t priority, uint32_t code_pages, uint32_t data_pages) {
    ...
    // 检查页数是否超过系统限制
    uint32_t total_pages = code_pages + data_pages + 10;  // 预留堆栈页
    if (total_pages > PHYSICAL_PAGES) {
        printf("进程总页数 %u 超过系统限制 %u\n", total_pages, PHYSICAL_PAGES);
        return NULL;
    }
    ...
    for (uint32_t i = 0; i < code_pages + data_pages; i++) {
        ...
        if (frame == (uint32_t)-1) {
            printf("分配页框失败，尝试选择受害者页面\n");
            // 选择受害者页面
            frame = select_victim_frame();
            if (frame == (uint32_t)-1) {
                printf("无法选择受害者页面，进程创建失败\n");
                // 释放已分配的帧
