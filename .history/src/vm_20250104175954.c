...
bool handle_page_fault(PCB* process, uint32_t virtual_page) {
    ...
    if (frame == (uint32_t)-1) {
        ...
        victim_pte->flags.swap_index = vm_manager.swap_blocks[victim_pte->flags.swap_index].swap_index;
        printf("更新被置换页面的页表项：present=%d, swapped=%d, swap_index=%u\n",
               victim_pte->flags.present, victim_pte->flags.swapped, victim_pte->flags.swap_index);
