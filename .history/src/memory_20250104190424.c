...
uint32_t select_victim_page(PCB* process) {
    ...
    for (uint32_t pid = 1; pid <= MAX_PROCESSES; pid++) {
        ...
        if (physical_ratio > 40.0f) {
            ...
            for (uint32_t i = 0; i < page_count; i++) {
                printf("尝试置换进程 %u 的页面（虚拟页号=%u，物理页框=%u）\n",
                       pid, pages[i].page_num, pages[i].frame_num);
    ...
    if (max_pages_process && max_present_pages > 0) {
        ...
        // 尝试每个页面
        for (uint32_t i = 0; i < page_count; i++) {
            printf("尝试强制置换进程 %u 的页面（虚拟页号=%u，物理页框=%u）\n",
                   max_pages_process->pid, pages[i].page_num, pages[i].frame_num);
