...
uint32_t select_victim_page(PCB* process) {
    ...
    for (uint32_t pid = 1; pid < scheduler.next_pid; pid++) {
        ...
        printf("进程 %u 当前物理页面比例: %.2f%%, 物理页面数量: %u\n", 
               pid, physical_ratio, present_pages);
        
        // 如果进程物理页面比例低于15%且物理页面数量少于8个，则跳过
        if (physical_ratio <= 15.0f && present_pages < 8) {
            printf("进程 %u 的物理页面比例和数量都较低，跳过\n", pid);
            continue;
        }
