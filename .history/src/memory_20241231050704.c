...
uint32_t allocate_frame(uint32_t process_id, uint32_t virtual_page_num) {
    printf("\n=== 开始分配页框 ===\n");
    printf("进程 %u 请求虚拟页面 %u\n", process_id, virtual_page_num);
...
void print_memory_map(void) {
    ...
    for (uint32_t i = 0; i < PHYSICAL_PAGES; i++) {
        if (memory_manager.frames[i].is_allocated) {
            printf("[%3u] PID=%-3u 虚拟页号=0x%04x\n",
                   i,
                   memory_manager.frames[i].process_id,
                   memory_manager.frames[i].virtual_page_num);
        }
    }
    ...
    printf("已用内存：%u 字节 (%.1f%%)\n", 
           (PHYSICAL_PAGES - memory_manager.free_frames_count) * PAGE_SIZE,
           (float)(PHYSICAL_PAGES - memory_manager.free_frames_count) * 100 / PHYSICAL_PAGES);
...
uint32_t select_victim_frame(void) {
    printf("\n=== 开始选择牺牲页框 ===\n");
    printf("当前已分配页框数: %u\n", PHYSICAL_PAGES - memory_manager.free_frames_count);
    ...
    for (uint32_t i = 0; i < PHYSICAL_PAGES; i++) {
        if (memory_manager.frames[i].is_allocated) {
            ...
            if (process) {
                printf("检查页框 %u: PID=%u, 页号=%u, 页表大小=%u, present=%d\n",
                       i, process->pid, memory_manager.frames[i].virtual_page_num,
                       process->page_table_size,
                       process->page_table[memory_manager.frames[i].virtual_page_num].flags.present);
            }
            uint32_t page_num = memory_manager.frames[i].virtual_page_num;
            printf("检查页框 %u: PID=%u, 页号=%u", i, 
                   memory_manager.frames[i].process_id, page_num);
            ...
            if (process && page_num < process->page_table_size && 
                ...
                if (process->page_table[page_num].last_access_time < oldest_access) {
                    ...
                    victim_frame = i;
                    printf(" - 更新为当前最佳候选 (访问时间: %lu)\n", oldest_access);
                } else {
                    printf(" - 保留现有候选 (访问时间: %lu > %lu)\n", 
                           process->page_table[page_num].last_access_time, oldest_access);
                }
            } else {
                printf(" - 跳过 (进程%s, 页表大小%s, present=%s)\n",
                       process ? "有效" : "无效",
                       (process && page_num < process->page_table_size) ? "有效" : "无效",
                       (process && page_num < process->page_table_size && 
                        process->page_table[page_num].flags.present) ? "true" : "false");
            }
        }
    }
    
    printf("选择结果: %s\n", victim_frame == (uint32_t)-1 ? 
           "没有找到合适的牺牲页框" : "找到牺牲页框");
    return victim_frame;
}
