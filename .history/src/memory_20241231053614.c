...
// 在 FrameInfo 结构体中添加时间戳
typedef struct {
    bool is_allocated;
    uint32_t process_id;
    uint32_t virtual_page_num;
    bool is_swapping;
    uint64_t last_access_time;  // 添加访问时间戳
} FrameInfo;
...
void free_frame(uint32_t frame_number) {
    ...
    // 先获取进程信息并更新页表
    PCB* process = get_process_by_pid(memory_manager.frames[frame_number].process_id);
    if (process) {
        uint32_t page_num = memory_manager.frames[frame_number].virtual_page_num;
        if (page_num < process->page_table_size) {
            process->page_table[page_num].flags.present = false;
            process->page_table[page_num].frame_number = (uint32_t)-1;
        }
    }
...
uint32_t select_victim_frame(void) {
    ...
    for (uint32_t i = 0; i < PHYSICAL_PAGES; i++) {
        if (memory_manager.frames[i].is_allocated) {
            ...
            if (process) {
                ...
                printf("- Present: %d\n", 
                       page_num < process->page_table_size ? 
                       process->page_table[page_num].flags.present : -1);
    ...
    printf("\n选择结果: %s\n", 
           victim_frame == (uint32_t)-1 ? "没有找到合适的牺牲页框" : 
           "找到牺牲页框");
