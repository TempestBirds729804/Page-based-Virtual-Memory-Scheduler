...
void destroy_process(PCB* process) {
    ...
    for (uint32_t i = 0; i < PHYSICAL_PAGES; i++) {
        if (memory_manager.frames[i].process_id == process->pid) {
            printf("释放页框 %u\n", i);
            printf("页框状态：allocated=%d, pid=%u, page=%u\n",
                   memory_manager.frames[i].is_allocated,
...
PCB* create_process(uint32_t priority) {
    ...
    // 初始化页表
    new_process->page_table = (PageTableEntry*)calloc(MAX_PAGES_PER_PROCESS, sizeof(PageTableEntry));
    new_process->page_table_size = MAX_PAGES_PER_PROCESS;
    ...
    for (uint32_t i = 0; i < MAX_PROCESSES; i++) {
        if (processes[i].state == PROCESS_TERMINATED) {
            // 正确复制进程信息
            processes[i].pid = new_process->pid;
            processes[i].priority = new_process->priority;
            processes[i].state = new_process->state;
            processes[i].time_slice = new_process->time_slice;
            processes[i].page_table = new_process->page_table;
            processes[i].page_table_size = new_process->page_table_size;
            stored = true;
            printf("进程 %u 已存入进程数组位置 %u\n", new_process->pid, i);
            break;
        }
    }
    
    if (!stored) {
        printf("警告：无法将进程存入进程数组\n");
