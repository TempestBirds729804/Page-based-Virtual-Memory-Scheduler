...
void destroy_process(PCB* process) {
    ...
    // 从就绪队列移除
    for (int i = 0; i < 3; i++) {
        if (scheduler.ready_queue[i] == process) {
    ...
    for (uint32_t i = 0; i < MAX_PROCESSES; i++) {
        if (processes[i].pid == process->pid) {
            processes[i].state = PROCESS_TERMINATED;
            processes[i].page_table = NULL;  // 避免悬空指针
            scheduler.total_processes--;
            break;
        }
    }
...
PCB* create_process(uint32_t priority) {
    ...
    // 将进程存入数组
    bool stored = false;
    uint32_t process_index = 0;  // 保存进程在数组中的位置
    ...
    return &processes[process_index];  // 返回数组中的进程实例
}
